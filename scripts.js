...
function showProducts() {
  for (const product of products) {
    const productInfo = `#${product.id} ${formatProduct(product)}`;
...
function addProductToCart() {
  const productId = prompt('Vörusnúmer (ID):');
  ...
  const quantityToAdd = prompt('Fjöldi:');
  if (!validateInteger(quantityToAdd, 1, 99)) {
    console.error('Fjöldi er ekki löglegur, lágmark 1 og hámark 99.');
    return;
  }

  const cartLine = cart.lines.find(line => line.product.id === productToAdd.id);
  if (cartLine) {
    cartLine.quantity += parseInt(quantityToAdd, 10);
  } else {
    cart.lines.push({ product: productToAdd, quantity: parseInt(quantityToAdd, 10) });
  }
}
...
function showCart() {
  if (cart.lines.length === 0) {
    ...
    for (const cartLine of cart.lines) {
      const lineInfo = `${formatProduct(cartLine.product, cartLine.quantity)}`;
...
function checkout() {
  ...
  const customerName = prompt('Nafn:');
  const customerAddress = prompt('Heimilisfang:');
  
  if (!customerName || !customerAddress) {
    console.error('Vantar nafn og heimilisfang');
    return;
  }
